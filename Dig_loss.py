#this is our DigNet loss function
...
def Digloss_contrastive(eye,f1,f2):
    ...
    # print(triplet_minM)
    triplet_minM = torch.div(torch.clamp((triplet_minM - 1
                                         ),max=0),divide)

    loss = (torch.sum(torch.clamp(D_diag,min=0)) - torch.sum(triplet_minM))/l
    return loss

def Digloss_triplet(eye,f1,f2):
    l = int(f1.size(0))
    eyes = eye[0:l,0:l]
