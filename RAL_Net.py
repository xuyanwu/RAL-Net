...
parser.add_argument('--data-root', default='/mnt/Brain/wug/Brown_data/', type=str, metavar='D',
...
parser.add_argument('--loss-type', type=str, default="RAL_loss",
                    help='type of training loss: "RAL_loss","Hard_loss"')
...
def main(traindata,testdata1,testdata2):
    if args.augmentation == True:
        saving_root = "RAL_net_result_models/"+ args.loss_type + "_" + str(args.epochs) + "_" + str(args.n_pairs) + \
    ...
    for epoch in range(start, end):
        ...
        torch.save(model, saving_root + "/" + args.loss_type + "_" + str(epoch) + "_" + str(args.n_pairs) + \
    ...
    test(model, create_test_loaders(testdata1,args.batch_size,args.data_root),traindata,testdata1,saving_root)
    time.sleep(10)
    test(model, create_test_loaders(testdata2,args.batch_size,args.data_root),traindata,testdata2,saving_root)
